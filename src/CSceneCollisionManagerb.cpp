...
{
	if (!selector || radius.X == 0.0f || radius.Y == 0.0f || radius.Z == 0.0f || slidingSpeed == 0.0f )
		return position;
...
inline void CSceneCollisionManager::testTriangleIntersection(SCollisionData* colData,
                                                         ...
                                                         SCollisionData &colData, core::vector3df pos, core::vector3df vel)
{
	...
	/*if (triangleCnt) {
	   ...
	   box.MaxEdge.X,box.MaxEdge.Y,box.MaxEdge.Z,colData.eRadius.X,colData.eRadius.Y,colData.eRadius.Z);
	...
	f32 b = newVelocityVector.X * newVelocityVector.X +
	        newVelocityVector.Y * newVelocityVector.Y +
	        newVelocityVector.Z * newVelocityVector.Z;
	f32 lsqrt = FastSqrt(b);


	if (lsqrt < veryCloseDistance)
		return newBasePoint;

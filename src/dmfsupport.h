...
// For uses in engine core and only for engine related uses I release this library under zlib license.
                   ...
                   dmfHeader & header)
{
	...
	header.dmfVersion = (f32)atof(temp1[0].c_str());  //save version
	temp.clear();
	temp1.clear();
	...
	header.dmfShadow = (f32)atof(temp[2].c_str());
		...
		if(!(wat1[0]==String("water") && wat[2]==String("0")))
		{
			for(int j=0; j<fac; j++)
			{
				...
				for(int k=0; k<vert; k++)
				{
					...
					//copy x,y,z values
					vertices[vert_cnt].pos[0] = (f32)atof(temp1[0].c_str());
					vertices[vert_cnt].pos[1] = (f32)atof(temp1[1].c_str());
					vertices[vert_cnt].pos[2] = (f32)-atof(temp1[2].c_str());
					//get uv coords for tex and light if any
					vertices[vert_cnt].tc[0] = (f32)atof(temp[2+vert+(2*k)].c_str());
					vertices[vert_cnt].tc[1] = (f32)atof(temp[2+vert+(2*k)+1].c_str());
					tmp_sz=temp.size();
					vertices[vert_cnt].lc[0] = (f32)atof(temp[tmp_sz-(2*vert)+(2*k)].c_str());
					vertices[vert_cnt].lc[1] = (f32)atof(temp[tmp_sz-(2*vert)+(2*k)+1].c_str());
					vert_cnt++;
					temp1.clear();
				}
...
bool GetDMFLights(StringList RawFile /**<StringList representing a DMF file.*/,
	...
	for(i=0; i<lit; i++)
	{
		...
		if(atoi(temp[0].c_str())==1){
			...
			if(temp1[0]==String("dynamic"))
			{
				lights[d_lit].radius = (f32)atof(temp[4].c_str());
				lights[d_lit].pos[0] = (f32)atof(temp[5].c_str());
				lights[d_lit].pos[1] = (f32)atof(temp[6].c_str());
				lights[d_lit].pos[2] = (f32)-atof(temp[7].c_str());
			...
			for (int i=0; i<(int)userinfo.size(); i++)
			{
				...
				case 2:
					if(atof(userinfo[2].c_str()))
						waveheight = (f32)atof(userinfo[2].c_str());
					break;
				case 3:
					if(atof(userinfo[3].c_str()))
						wavespeed = (f32)atof(userinfo[3].c_str());
					break;
				case 4:
					if(atof(userinfo[4].c_str()))
						wavelength = (f32)atof(userinfo[4].c_str());

...
void CQ3LevelMesh::loadVisData(tBSPLump* l, io::IReadFile* file)
{
	...
	file->seek(l->offset);
	file->read(&VisData->numOfClusters, 4);
	file->read(&VisData->bytesPerCluster, 4);

	// visdata buffer size
	int sz = VisData->numOfClusters*VisData->bytesPerCluster;

	char *buffer = (char*)malloc(sz);

	file->read(VisData->pBitsets, sz);
}
	...
	for (i=0; i<NumFaces; ++i)
	{
		...
		case 1:         // normal polygons
			...
			for (s32 tf = 0; tf < Faces[i].numOfVerts; tf++)
			{
				...
				tvtx.Color.set(255,255,255,255);
				tvtx.Pos.X = v->vPosition[0];
				tvtx.Pos.Y = v->vPosition[2];
				tvtx.Pos.Z =  v->vPosition[1];
				tvtx.TCoords.X = v->vTextureCoord[0];
				tvtx.TCoords.Y = v->vTextureCoord[1];
				meshBuffer->Vertices.push_back( tvtx );
				meshBuffer->Indices.push_back( idx );
				meshBuffer->Indices.push_back( idx + tf + 1);
				meshBuffer->Indices.push_back( idx + tf + 2);

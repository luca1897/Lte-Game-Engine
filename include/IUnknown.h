...
namespace engine
{
	...
	Most objects of the engine
	Engine are derived from IUnknown, and so they are reference counted.

	When you create an object in the engine engine, calling a method
	...
	*/
	...
	public:
		...
		//! with its methods grab() and drop(). Most objects of the engine
		//! Engine are derived from IUnknown, and so they are reference counted.
		//!
		//! When you create an object in the engine engine, calling a method
		//! which starts with 'create', an object is created, and you get a pointer
		...
		bool drop()
		{
			_engine_DEBUG_BREAK_IF(ReferenceCounter <= 0) // someone is doing bad reference counting.

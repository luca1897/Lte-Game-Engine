...
namespace scene
{
	...
	*/
	...
	public:
		 ...
		 *      lighting. engine can directly import .csm files thanks to the engineCSM library 
		 *      created by Saurav Mohapatra which is now integrated directly in engine. 
		 ...
		 *        at http://collada.org. engine can import COLLADA files by using the 
		 ...
		 *        Salvatore for his work and for allowing me to use his code in engine and put it under engine's 
		 ...
		 *      include skeletal animations and engine is able to play and display them. 
		 *      Currently, engine only supports text encoded .x files.</TD>
		 *  </TR>
		 *  <TR> 
		 *    <TD>Maya (.obj)</TD>
		 *    <TD>Most 3D software can create .obj files which contain static geometry without 
		 *      material data. This importer for engine can load them directly. </TD>
		 *  </TR>
		 *  <TR> 
		 *    <TD>Milkshape (.ms3d)</TD>
		 *    <TD>.MS3D files contain models and sometimes skeletal animations from the 
		 *      Milkshape 3D modeling and animation software. This importer for engine 
		 ...
		 *        engine can read and display those files directly. This loader was written 
		 ...
		 *        directly by engine. OCT files<br>
		 ...
		 *        engine can directly read .lmts files thanks to<br>
		 ...
		 *        - The TGAs generated by LMTools don't work in engine for some reason (the 
		 ...
		 *        prelighted levels. engine can read .pk3 and .bsp files directly and thus 
		 ...
		 *      <TD>Quake 2 models are characters with morph target animation. engine can 
		 ...
		 *  \endcode
		 *  If you would like to implement and add your own file format loader to engine,
		...
		//! Adds a scene node for rendering using bsp tree to the scene graph.
		

		virtual ISceneNode* addBspSceneNode(void* mesh, ISceneNode* parent=0, 
			s32 id=-1, s32 minimalPolysPerNode=128) = 0;
		...
		/** Add a camera scene node fps controlled by the control keys of the PSP. There are three
        style of this camera, see the moveStyle for further information.
        The first moveStyle is default and sets that you can move the camera with the d-pad and rotate it with the analog,
        for all move styles you can set a custom keymap. A custom keymap let you to use your prefered controls to move/rotate
        the camera. For example.
		 ...
		 \endcode
     This code adds a camera scene node fps with a custom keymap to move the camera with 
     triangle, cross, square and circle.

     The better moveStyle for FPS games is the third (moveStyle = 2), it lets you to use the
     analog to move and rotate the camera: with analog left you rotate left, with analog right you rotate
     right, with analog up you move forward and with analog down you move backward. 
     ...
     \param moveStyle: sets the movement style for the camera:
     0 = Analog to rotate the camera, DPad to move the camera. Custom keymaps can be used to replace the movement
     controls.
     1 = Analog to move the camera. DPad to rotate the camera. Custom keymaps can be used to replace the   
     controls to rotate the camera.
     2 = Analog left and right rotate the camera, analog up and down move the camera. Custom actions
     EKA_MOVE_FORWARD and EKA_MOVE_BACKWARD rotate up and down the camera; EKA_STRAFE_LEFT and EKA_STRAFE_RIGHT
     move the camera left and right. By default you can rotate the camera up and down with D-PAD UP and DOWN 
     and to move the camera left and right: D-PAD LEFT and RIGHT.
		 \return Returns a pointer to the interface of the camera if successful, otherwise 0.
		 This pointer should not be dropped. See IUnknown::drop() for more information. */
		virtual ICameraSceneNode* addCameraSceneNodeFPS(ISceneNode* parent = 0,
			f32 rotateSpeed = 100.0f, f32 moveSpeed = 500.0f, s32 id=-1,
			SKeyMap* keyMapArray=0, s32 keyMapSize=0, bool noVerticalMovement=false,int moveStyle = 0) = 0;
		...
		The original/main scene manager will still be there and accessible via engineDevice::getSceneManager().

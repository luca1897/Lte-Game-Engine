...
namespace core
{
	...
	/* Matrix4 is mainly used by the engine engine for doing transformations.
  ...
  inline void matrix4::toGLMatrix(float *glmat)
  {
	...
	{
		f32 cr = FastCos( rotation.X );
		f32 sr = FastSin( rotation.X );
		f32 cp = FastCos( rotation.Y );
		f32 sp = FastSin( rotation.Y );
		f32 cy = FastCos( rotation.Z );
		f32 sy = FastSin( rotation.Z );
	...
	inline core::vector3df matrix4::getRotationDegrees() const
	{
		...
		{ 
			rotx = mat(2,2) / C; 
			roty = mat(2,1)  / C; 
			X = FastAtan2( roty, rotx ) * GRAD_PI; 
			rotx = mat(0,0) / C; 
			roty = mat(1,0) / C; 
			Z = FastAtan2( roty, rotx ) * GRAD_PI; 
		} 
		else 
		{ 
			X  = 0.0f; 
			rotx = mat(1,1); 
			roty = -mat(0,1); 
			Z  = FastAtan2( roty, rotx ) * (f32)GRAD_PI; 
		} 
...
inline bool matrix4::makeFullInverse()
	...
	{
	    f32 h = (f32)( FastCos(fieldOfViewRadians/2) /  FastSin(fieldOfViewRadians/2));
		f32 w = h / aspectRatio;
